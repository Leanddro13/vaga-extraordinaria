...
pacotes_debian_base="pulseaudio-utils dbus-python python3-dbus google-chrome-stable"
pacotes_rhel_base="pulseaudio-utils dbus-python python3-dbus google-chrome-stable"
...
if [[ "$distro" == "ubuntu" || "$distro" == "debian" ]]; then
	apt update -y
	apt install $pacotes_debian_base

elif [[ "$distro" == "rhel" || "$distro" == "fedora" || "$distro" == "centos" ]]; then
	dnf update -y
	dnf install $pacotes_rhel_base
